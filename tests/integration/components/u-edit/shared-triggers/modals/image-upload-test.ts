...
const URL: string = 'https://www.test.com/350x150.png';
...
module('Integration | Component | u-edit/shared-triggers/modals/image-upload', function (hooks) {
  ...
  test('the modal title is correct', async function (assert) {
    ...
    assert.dom('header .title').hasText('Add image');
  });
  ...
  test("the alert info isn't rendered", async function (assert) {
    ...
    assert.dom('.upf-alert.upf-alert--info').doesNotExist();
  });
  ...
  module('for the url input', () => {
    test('the placeholder is correct', async function (assert) {
      ...
      assert.dom('.upf-input').hasAttribute('placeholder', 'https://example.org/image.png');
    });
